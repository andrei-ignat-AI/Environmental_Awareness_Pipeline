...
EXPORT_SINGLE_BOX_PER_OBJECT = True
REMOVE_TABLE_IN_MUJOCO_EXPORT = os.environ.get("MUJOCO_REMOVE_TABLE", "1").strip().lower() not in {"0", "false", "no", "off"}
INCLUDE_TABLE_IN_MUJOCO_EXPORT = not REMOVE_TABLE_IN_MUJOCO_EXPORT
MERGE_OCCLUSIONS_WITH_OBJECTS_FOR_EXPORT = True
...
MANUAL_DIAGNOSTIC_SHIFT = np.array([-0.05, -0.05, -0.05], dtype=np.float64)
INCLUDE_ROBOT = True
...
def component_to_mesh_export_record(component, record, room_dimensions):
    ...
    object_index = int(record["object_index"])
    name = f"{kind}_{object_index:02d}_hull"
...
def save_export_files(result):
    ...
    if EXPORT_GEOM_MODE == "convex_hull":
        export_records = [
            ...
            for component, record in zip(result["components"], result["box_records"])
        ]
...
def export_voxels_from_mujoco(
    ...
    for i in range(model.ngeom):
        ...
        combined_mesh += sub_mesh.subdivide_midpoint(number_of_iterations=2)
    ...
    scene.add_triangles(o3d.t.geometry.TriangleMesh.from_legacy(combined_mesh))

    room_origin_o3d = np.array([
        -decomposition.default_room_dimensions[0] / 2,
        0.0,
        -decomposition.default_room_dimensions[2] / 2,
    ], dtype=np.float64)
    min_b_o3d = mujoco_to_open3d_vector(combined_mesh.get_min_bound() - voxel_size)
    max_b_o3d = mujoco_to_open3d_vector(combined_mesh.get_max_bound() + voxel_size)
    min_idx = np.floor((np.minimum(min_b_o3d, max_b_o3d) - room_origin_o3d) / voxel_size).astype(int)
    max_idx = np.ceil((np.maximum(min_b_o3d, max_b_o3d) - room_origin_o3d) / voxel_size).astype(int)
    ...
    query_points = o3d.core.Tensor(np.asarray([open3d_vector_to_mujoco(point) for point in centers_o3d], dtype=np.float32))
    ...
    touch_margin = (voxel_size * np.sqrt(3) / 2) + 0.002
    inside_points_o3d = centers_o3d[np.logical_or(occupancy, signed_distance <= touch_margin)] + MANUAL_DIAGNOSTIC_SHIFT
    if len(inside_points_o3d) == 0:
    ...
    os.makedirs(export_dir, exist_ok=True)
    file_sample_name = os.path.basename(sample_name)
    indices = np.floor((inside_points_o3d - room_origin_o3d) / voxel_size).astype(np.int32)
    np.save(os.path.join(export_dir, f"{file_sample_name}_voxel_indices.npy"), indices)

    point_cloud = o3d.geometry.PointCloud()
    point_cloud.points = o3d.utility.Vector3dVector(inside_points_o3d)
    success = o3d.io.write_point_cloud(os.path.join(export_dir, f"{file_sample_name}_voxels.ply"), point_cloud)
    if success:
