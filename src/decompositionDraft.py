...
MERGE_OCCLUSIONS_WITH_OBJECTS = True
INCLUDE_DETACHED_OCCLUSION_ZONES = True
BUILD_OCCLUSION_ZONE_BOXES = False
SHOW_OCCLUSION_ZONE_BOXES = False
...
OCCLUSION_MAX_COMPONENT_VOXELS = 3000
...
class RoomVoxelGrid():
    ...
    def convert_to_numpy(self,debug=False):
        ...
        for voxel in voxels:
            x_idx = int(voxel.grid_index[0])
            y_idx = int(voxel.grid_index[1])
            z_idx = int(voxel.grid_index[2])
            if 0 <= x_idx < voxel_array.shape[0] and 0 <= y_idx < voxel_array.shape[1] and 0 <= z_idx < voxel_array.shape[2]:
                voxel_array[x_idx, y_idx, z_idx] = True
...
def compute_occlusion_grid(depth_image_paths, intrinsics, extrinsics, resolution=default_voxel_size, room_dimensions=default_room_dimensions):
    ...
    for img_path, intrinsic, extrinsic in zip(depth_image_paths, intrinsics, extrinsics):
        ...
        near_clip, _ = azurion_dataset.depth_clip_for_raw_path(img_path)
        ...
        in_frustum = (z_cam > 0) & (ui >= 0) & (ui < w) & (vi >= 0) & (vi < h)
        ...
        free_flat |= in_frustum & surf_valid & (z_cam < surf_depth)
...
def draft_colorize_components(component_points, component_indices=None, occlusion_mask=None):
    geometries = []
    for index, points in enumerate(component_points[:MAX_COMPONENTS_TO_DRAW]):
        color = draft_component_color(index)
...
def draft_build_component_convex_hulls(
    ...
    for object_index, component in enumerate(component_indices[:MAX_COMPONENTS_TO_DRAW]):
        ...
        mesh.compute_vertex_normals()
        color = draft_component_color(object_index)
    ...
    return hull_meshes, hull_records


def draft_log_convex_hulls(hull_records):
    for record in hull_records:
        print(
            f"  entity {record['object_index']:02d} convex hull: "
...
def draft_process_sample(dataset, sample_index):
    ...
    if SHOW_COMPONENT_CONVEX_HULLS:
        hull_meshes, hull_records = draft_build_component_convex_hulls(
            components,
            default_voxel_size,
            room_dimensions,
            margin_m=CONVEX_HULL_MARGIN_M,
    ...
    if BUILD_OCCLUSION_ZONE_BOXES and not MERGE_OCCLUSIONS_WITH_OBJECTS:
    ...
    elif BUILD_OCCLUSION_ZONE_BOXES:
        print("occlusion zone boxes produced: 0 (already included in entity boxes)")
...
def draft_show_interactive_sample_browser(dataset):
    ...
    load_sample(DEFAULT_SAMPLE_INDEX)
...
def draft_run_decomposition_smoke_test():
    ...
    if OPEN_INTERACTIVE_VIEW:
        draft_show_interactive_sample_browser(dataset)
    else:
        result = draft_process_sample(dataset, DEFAULT_SAMPLE_INDEX)
